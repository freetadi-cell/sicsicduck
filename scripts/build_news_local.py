...
def card(a, show_image, lead_index=0):
    ...
    # 圖片：優先用已存嘅 image_url，冇先按主題配並存回
    if show_image:
        stored_img = a.get("image_url", "")
        if stored_img:
            image = esc(stored_img)
        else:
            image = esc(theme_for(a))
            a["image_url"] = image  # 存回 news.json
        image_block = f'''<div class="article-image"><img src="{image}" alt="" loading="lazy"></div>'''
    else:
        image_block = ""

    summary = load_summary(aid)
    # 存 rewritten summary 到 news.json（下次 build 直接用）
    if summary and not a.get("rewritten"):
        a["rewritten"] = summary
...
'''
    ...
    # Save image_url back to news.json（已配過嘅圖唔再換）
    try:
        with open(DATA_DIR / "news.json", encoding="utf-8") as f:
            nj = json.load(f)
        id_map = {a.get("id"): a for a in articles if a.get("id")}
        for na in nj.get("articles", []):
            if na.get("id") in id_map and id_map[na["id"]].get("image_url"):
                na["image_url"] = id_map[na["id"]]["image_url"]
        with open(DATA_DIR / "news.json", "w", encoding="utf-8") as f:
            json.dump(nj, f, ensure_ascii=False, indent=None)
    except Exception as e:
        print(f"   ⚠️ save image_url failed: {e}")

    out_path.write_text(out, encoding="utf-8")
    re_modal = re.compile(r'class="news-modal"')
    modal_count = len(re_modal.findall(modals)) if modals else 0
    print(f"✅ 生成 {out_path.name}（{len(articles)} 篇，{modal_count} 個 modal）")
