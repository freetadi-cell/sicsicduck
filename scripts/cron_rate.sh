...
if tail -n +"$((LINES_BEFORE+1))" /tmp/sicsicduck-rate.log | grep -q "Git commit and push completed"; then
  openclaw message send --channel telegram -t telegram:885017126 -m "🏦 利率更新完成 ✅" 2>/dev/null || true
else
  openclaw message send --channel telegram -t telegram:885017126 -m "❌ 利率更新失敗，請檢查 /tmp/sicsicduck-rate.log" 2>/dev/null || true
fi
