...
def interleave_sources(articles, max_consecutive=2):
    """混合排列：同一來源最多連續 max_consecutive 篇"""
    ...
    while queues:
        # 睇最後 N 篇嘅來源
        recent_srcs = [r.get("source_name", "unknown") for r in result[-max_consecutive:]]
        # 揀一個唔會造成連續超標嘅來源
        picked = None
        for src in list(queues.keys()):
            if recent_srcs.count(src) < max_consecutive:
