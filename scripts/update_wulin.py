...
2. dialogue 用白話文（與金庸小說一致嘅文風），完全符合角色性格同最近經歷，三句
...
10. 所有文字必須使用繁體中文（正體字），嚴禁使用簡體字（例如：戰≠战、門≠门、國≠国、發≠发、飛≠飞）
11. 只輸出 JSON，唔好 markdown fence、唔好解釋"""
...
try:
    ...
    # fallback: manual mapping (less complete)
    _S2T = {
        '孙':'孫','时':'時','后':'後','杀':'殺','将':'將','两':'兩','双':'雙',
        '对':'對','关':'關','还':'還','进':'進','开':'開','过':'過','问':'問',
        '间':'間','门':'門','东':'東','车':'車','书':'書','机':'機','号':'號',
        '战':'戰','军':'軍','从':'從','与':'與','为':'為','实':'實','发':'發',
        '动':'動','应':'應','现':'現','让':'讓','说':'說','认':'認','请':'請',
        '该':'該','设':'設','议':'議','记':'記','读':'讀','转':'轉','达':'達',
        '运':'運','连':'連','远':'遠','边':'邊','选':'選','队':'隊','难':'難',
        '风':'風','马':'馬','龙':'龍','铁':'鐵','总':'總','统':'統','结':'結',
        '经':'經','给':'給','约':'約','级':'級','组':'組','织':'織','终':'終',
        '练':'練','继':'繼','见':'見','论':'論','调':'調','谋':'謀','谁':'誰',
        '证':'證','质':'質','败':'敗','财':'財','资':'資','卖':'賣','买':'買',
        '万':'萬','专':'專','众':'眾','陆':'陸','体':'體','术':'術','传':'傳',
        '伤':'傷','侠':'俠','儿':'兒','宁':'寧','决':'決','离':'離','准':'準',
        '启':'啟','几':'幾','凤':'鳳','创':'創','刚':'剛','剧':'劇','劳':'勞',
        '势':'勢','卫':'衛','变':'變','叶':'葉','国':'國','图':'圖','团':'團',
        '场':'場','坚':'堅','块':'塊','壮':'壯','声':'聲','处':'處','备':'備',
        '复':'復','夺':'奪','奋':'奮','学':'學','宝':'寶','宾':'賓','尔':'爾',
        '尘':'塵','层':'層','属':'屬','币':'幣','帮':'幫','向':'嚮','厉':'厲',
        '联':'聯','广':'廣','庆':'慶','庄':'莊','库':'庫','弹':'彈','强':'強',
        '归':'歸','征':'徵','忆':'憶','怀':'懷','恶':'惡','惩':'懲','戏':'戲',
        '执':'執','扫':'掃','报':'報','担':'擔','损':'損','换':'換','敌':'敵',
        '斗':'鬥','斩':'斬','断':'斷','无':'無','显':'顯','条':'條','松':'鬆',
        '极':'極','构':'構','标':'標','梦':'夢','检':'檢','欢':'歡','歼':'殲',
        '残':'殘','毁':'毀','沟':'溝','浅':'淺','测':'測','济':'濟','灯':'燈',
        '炉':'爐','烟':'煙','爱':'愛','犹':'猶','猎':'獵','环':'環','画':'畫',
        '畅':'暢','疗':'療','积':'積','稳':'穩','竞':'競','笔':'筆','签':'簽',
        '简':'簡','粮':'糧','紧':'緊','类':'類','锋':'鋒','镇':'鎮','钟':'鐘',
        '钱':'錢','银':'銀','铜':'銅','钢':'鋼','内':'內','兴':'興','彻':'徹',
    }
    def to_traditional(text):
        if not isinstance(text, str):
            return text
...
def apply_delta(world, delta, new_day):
    ...
    for e in delta.get("events") or []:
        if isinstance(e, dict) and e.get("text"):
            events.append({
                "day": new_day,
                "text": to_traditional(str(e["text"])),
        ...
        for o in arr if isinstance(arr, list) else []:
            if isinstance(o, dict) and o.get("say") and o.get("reply"):
                try:
                    ...
                    d = 0
                cleaned.append({"say": to_traditional(str(o["say"]))[:60],
                                "reply": to_traditional(str(o["reply"]))[:150],
                                "affinity_delta": d,
